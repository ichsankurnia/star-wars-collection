...
const People: React.FC<Props> = () => {
    ...
    const fetchPeople = async ({ pageParam }: any) => {
        const req = await fetch(`${BASE_URL}/people?page=${pageParam || 1}`)
    ...
    useEffect(() => {
        if (hasNextPage) {
            ...
            queryClient.prefetchInfiniteQuery(['people', nextPage], () => fetchPeople({ pageParam: nextPage }))
    ...
    return (
        <>
            ...
            {data?.pages?.map((page, i) =>
                <Fragment key={i}>
                    <p>Page {i + 1}</p>
                    {page.results?.map((item: any, key: number) =>
            ...
            <div className='px-5 flex flex-col'>
                <h1 className='font-bold text-xl'>People Collection</h1>
                <div className='grid grid-cols-1 md:grid-cols-2 gap-5 my-5'>
                    {data?.pages?.map((page, i) =>
                        <Fragment key={i}>
                            {/* <p className='text-xl font-semibold'>Page {i + 1}</p> */}
                            {page.results?.map((item: IPeople, key: number) =>
                                    ...
                                    <div className="flex flex-col">
                                        <div className="flex justify-between">
                                            <div className="flex items-center">
                                                <LazyLoadImage effect='blur' alt={`img-${item.name}`} src={peopleIcon} className="w-16 h-16 rounded-full" />
                                                <div className="ml-5 space-y-0.5">
                                                    <div className='font-semibold text-base'>{item.name}</div>
                                                    <Link to={item.homeworld.split(BASE_URL)[1]} className='truncate text-blue-500 hover:underline' onClick={(e)=>e.stopPropagation()}>
                                                        Homeworld {item.homeworld.split('/')[item.homeworld.split('/').length - 3]} {item.homeworld.split('/')[item.homeworld.split('/').length - 2]}
                                                    </Link>
                                                </div>
                                            </div>
                                        ...
                                        <div className="flex flex-col my-4">
                                            <div>{item.gender} | {item.height} cm</div>
                                            <div>Birthday {item.birth_year}</div>
                                        </div>
