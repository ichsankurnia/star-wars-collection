...
import thumbnail from '../asset/thumbnail-species.webp'
import GlobalContext from '../context/GlobalContext';

type Props = {};

const Species: React.FC<Props> = () => {
    ...
    const fetchData = async ({ pageParam }: any) => {
        const req = await fetch(`${BASE_URL}/species?page=${pageParam || 1}`)
    ...
    return (
        <>
            <div className='px-5 flex flex-col'>
                <h1 className='font-bold text-xl'>Species Collection</h1>
                <div className='grid grid-cols-1 md:grid-cols-2 gap-5 my-5'>
                    {data?.pages?.map((page, i) =>
                        <Fragment key={i}>
                            {/* <p className='text-xl font-semibold'>Page {i + 1}</p> */}
                            {page.results?.map((item: ISpecies, key: number) =>
                                    ...
                                    <div className="flex flex-col">
                                        <div className="flex justify-between">
                                            <div className="flex items-center">
                                                <LazyLoadImage effect='blur' alt={`img-${item.name}`} src={thumbnail} className="w-32 h-16 object-cover rounded-lg" />
                                        ...
                                        <div className="flex flex-col my-4">
                                            <div className='text-gray-500'>Classification <span className='font-medium text-gray-700'>{item.classification}</span></div>
                                        </div>
