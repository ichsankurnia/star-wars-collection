...
import { IStarships } from '../utils/interface';
import poster from '../asset/poster-starships.png'
...
const StarshipsDetail: React.FC<Props> = () => {
    const { pathname } = useLocation()

    const { result, loading, error } = useDetailCollection(pathname)
    const data = result as IStarships
    ...
    return (
        <>
            <div className='px-5 flex flex-col pb-10'>
                <h1 className='font-bold text-lg'><Link to='/starships' className='text-blue-500 text-xl hover:underline'>Starships Collection</Link> / Detail</h1>
                <div className='border p-5 rounded-2xl mt-5'>
                    ...
                    <div className='flex flex-col-reverse md:flex-row justify-between items-center relative'>
                        <div className='grid grid-cols-2 md:grid-cols-3 gap-y-4 gap-x-10 md:gap-x-16 md:text-base capitalize z-[1]'>
                            ...
                            <span className='detail-value col-span-1 md:col-span-2'>{data?.starship_class}</span>
                            ...
                            <span className='detail-value col-span-1 md:col-span-2'>{data?.MGLT} Megalight</span>
                            <span className='detail-label'>hyperdrive rating</span>
                            <span className='detail-value col-span-1 md:col-span-2'>{data?.hyperdrive_rating}</span>
                        ...
                        <div className='md:absolute top-0 right-0 mb-5 md:mb-0 -mt-2 z-0'>
                            <LazyLoadImage effect='blur' alt='' src={poster} className='w-40 md:w-[30rem]' />
                        </div>
                    </div>
                </div>
            </div>
