...
import thumbnail from '../asset/thumbnail-vehicles.png'
...
const Vehicles: React.FC<Props> = () => {
    ...
    const fetchData = async ({ pageParam }: any) => {
        const req = await fetch(`${BASE_URL}/vehicles?page=${pageParam || 1}`)
    ...
    return (
        <>
            <div className='px-5 flex flex-col'>
                <h1 className='font-bold text-xl'>Vehicles Collection</h1>
                <div className='grid grid-cols-1 md:grid-cols-2 gap-5 my-5'>
                    {data?.pages?.map((page, i) =>
                        <Fragment key={i}>
                            {/* <p className='text-xl font-semibold'>Page {i + 1}</p> */}
                            {page.results?.map((item: IVehicles, key: number) =>
                                    ...
                                    <div className="flex flex-col">
                                        <div className="flex justify-between">
                                            <div className="flex items-center">
                                                <LazyLoadImage effect='blur' alt={`img-${item.name}`} src={thumbnail} className="w-20 h-16 object-cover" />
